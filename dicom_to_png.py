...
from PyQt5.QtWidgets import QMainWindow, QApplication, QPushButton, QWidget, QLabel, QGridLayout, QPlainTextEdit, QFileDialog
...
def apply_LUT(img, hdr):
    ...
    LUT = {int(v):lut_data[j] for j, v in [(i, first_value+i) for i in range(len(lut_data))]}
...
class ConverterWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.configButton.setToolTip("Click to select the folder where PNG images should be placed.")
        ...
        self.instructions = QLabel("Drop DICOM files or directories\nhere to convert to PNG.", self) 
    ...
    def addFilesDialog(self):    
        ...
        files, _ = QFileDialog.getOpenFileNames(self,"Select DICOM Files", "","DICOM Files (*.dcm *.dicom *);;All Files (*)", options=options)
        new_items = {
            'files': files,
            'dirs': []
        }
    ...
    def dropEvent(self, event):
        ...
        self.setStatusBar("Added {} files and {} directories.".format(len(new_items['files']), len(new_items['dirs'])))
        self.processNewItems(new_items)
    ...
    def startThreads(self):
        ...
        if len(self.queue) == 0:
            self.setStatusBar("{} Files converted.  You can add more, or exit.".format(self.convertedCount))
            self.indicateThreadsRunning(False)
        ...
        for id in list(self.queue.keys()):
            ...
            del self.queue[id]
            self.working[worker_id] = (info, worker, thread, id)  # need to store worker and thread too otherwise will be gc'd
    ...
    def processNewItems(self, new_items):
        ...
        for fname in files:
            if fname in converting:
                ...
                self.abortWorkers()
                sys.exit(1)
            job_id = hashlib.sha1("{}{}".format(fname, time.process_time()).encode('ascii')).hexdigest()
            self.queue[job_id] = {
...
class ConversionWorker(QObject):
    ...
    def doConversion(self):
        ...
        try:
            output_file = "{}.png".format(os.path.splitext(os.path.basename(file_path))[0])
            ...
            self.sig_msg.emit('[FAIL]: Failed converting "{}" (#{})'.format(basename, thread_id))
            t = QThread.currentThread()
            if t.isRunning():
                t.quit() # This thread needs to quit!
            return
    ...
    def abort(self):
        if not self.abort_requested:
            self.sig_msg.emit('[!]: Cancelling conversion for "{}".'.format(self.task_info['basename']))
